...
var rankingByBrazilianStates = [];
var rankingByCountries = [];

var hashMapByYear = {};
var hashMapByEntryKey = {};
var hashMapByEntryType = {};
var hashMapByAuthor = {};
var hashMapByAffiliation = {};
var hashMapByApplication = {};
var hashMapByJournal = {};
...
function viewDataCountries(){
	$('#modal-view-data-countries').modal('show');
}

function viewDataBrazilianStates(){
	$('#modal-view-data-brazilian-states').modal('show');
}
...
function success(response){
	...
	plotNumberOfPublicationsByCountries(entries);
	plotNumberOfPublicationsByBrazilianStates(entries);
...
function parse(content){
	...
	messageSpin("Processing entries...");
...
function processEntry(entry){
	...
	generateRankingByBrazilianStatesAndCountries(entry);
}
...
function generateHashMaps(entry){

    if( ! hashMapByYear[entry.year]){
        hashMapByYear[entry.year] = [];
    }
    if( ! hashMapByEntryKey[entry.EntryKey]){
        hashMapByEntryKey[entry.EntryKey] = [];
    }
    if( ! hashMapByEntryType[entry.entryType]){
        hashMapByEntryType[entry.entryType] = [];
    }
    if( ! hashMapByApplication[entry.application]){
        hashMapByApplication[entry.application] = [];
    }
    if( ! hashMapByJournal[entry.journal]){
        hashMapByJournal[entry.journal] = [];
    }

    hashMapByYear[entry.year].push(entry);
    hashMapByEntryKey[entry.EntryKey].push(entry);
    hashMapByEntryType[entry.entryType].push(entry);
    hashMapByApplication[entry.application].push(entry);
    hashMapByJournal[entry.journal].push(entry);

    $.each(entry.author, function(index, author){
        if( ! hashMapByAuthor[author.last]){
            hashMapByAuthor[author.last] = [];
        }

        hashMapByAuthor[author.last].push(entry);
    });
}

function generateRankingByBrazilianStatesAndCountries(entry){
	...
	if(addresses.length <= 1 || addresses.length >= 4){
		console.error("The entry is wrong: "+entry.cite);
	}else{
		insertOrUpdate(rankingByCountries, addresses[addresses.length-1].trim());

		if(addresses[addresses.length-1].trim() == "Brazil"){
			insertOrUpdate(rankingByBrazilianStates, addresses[addresses.length-2].trim());
		}
	}
}
...
function plotNumberOfPublicationsByBrazilianStates(entries){
	sort(rankingByBrazilianStates);

	var categories = [];

	var data = [];

	$.each(rankingByBrazilianStates, function(key, entry){
		categories.push(entry.label)
		data.push(entry.count);

		appendRow("#table-view-data-brazilian-states", [(key+1), entry.label, entry.count]);
    });

	var series = [{name:"Number of Papers", data:data, color: "#8085e9"}];

	var options = {
		elementId: "#chart-brazilian-states",
		categories: categories,
		series: series,
		title: "Number of Publications by Brazilian State",
		viewData: viewDataBrazilianStates
	};

	plotBasicColumnChart(options);
}

function plotNumberOfPublicationsByCountries(entries){
	sort(rankingByCountries);
	...
	$.each(rankingByCountries, function(key, entry){
		...
		appendRow("#table-view-data-countries", [(key+1), entry.label, entry.count]);
    });

	var series = [{name:"Number of Papers", data:data, color: "#f15c80"}];

	var options = {
		elementId: "#chart-countries",
		categories: categories,
		series: series,
		title: "Number of Publications by Country",
		viewData: viewDataCountries,
	};

	plotBasicColumnChart(options);
}

function plotBasicColumnChart(options){
